this.JSON||(this.JSON={});
(function(){function b(a){return 10>a?"0"+a:a}function a(a){d.lastIndex=0;return d.test(a)?'"'+a.replace(d,function(a){var c=k[a];return"string"===typeof c?c:"\\u"+("0000"+a.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+a+'"'}function c(b,e){var d,q,r,k,s=f,n,m=e[b];m&&"object"===typeof m&&"function"===typeof m.toJSON&&(m=m.toJSON(b));"function"===typeof g&&(m=g.call(e,b,m));switch(typeof m){case "string":return a(m);case "number":return isFinite(m)?String(m):"null";case "boolean":case "null":return String(m);case "object":if(!m)return"null";
f+=h;n=[];if("[object Array]"===Object.prototype.toString.apply(m)){k=m.length;for(d=0;d<k;d+=1)n[d]=c(d,m)||"null";r=0===n.length?"[]":f?"[\n"+f+n.join(",\n"+f)+"\n"+s+"]":"["+n.join(",")+"]";f=s;return r}if(g&&"object"===typeof g)for(k=g.length,d=0;d<k;d+=1)q=g[d],"string"===typeof q&&(r=c(q,m))&&n.push(a(q)+(f?": ":":")+r);else for(q in m)Object.hasOwnProperty.call(m,q)&&(r=c(q,m))&&n.push(a(q)+(f?": ":":")+r);r=0===n.length?"{}":f?"{\n"+f+n.join(",\n"+f)+"\n"+s+"}":"{"+n.join(",")+"}";f=s;return r}}
...
function(){b(this).remove()})},l=null,p=null;(function(c){b.isArray(c)?(p=function(d,f){var e=new RegExp(b.ui.autocomplete.escapeRegex(string_ci_ai(d.term)));f(b.grep(c,function(b){return e.test(string_ci_ai(b[a.match_prop]||b))}))},l=function(d,f){var e=string_ci_ai(d),g=b.grep(c,function(b){return string_ci_ai(string_ci_ai(b[a.match_prop]||b))===e});if(1===g.length)return f(g[0]),!0;f()}):"string"===typeof c?(p=create_caching_source_fn(b,c,k,a.match_prop),l=function(c,d,f){var e=string_ci_ai(c);
if(k.content){var g=b.grep(k.content,function(b){return string_ci_ai(b[a.match_prop])===e});if(1===g.length){d(g[0]);return}}f||string_ci_ai(k.term||"")===e?d():p({term:e},function(a){l(c,d,!0)})}):(p=c,l=a.look_for_fn)})(e);var t=b("#add_"+c).click(function(d){d=h.data("chkid");var e=h.data("display"),k=h[0].value;d&&e&&e==k?(f(d,e),g()):l(h[0].value,function(d){d?(f(d.chkid,d[a.match_prop]),g()):0===b("#"+c+"_error").length&&(b("#"+c+"_new_input_table").before(b('<div class="ui-widget clearfix" style="margin: 0.25em;">\r\n                <div class="ui-state-error error-field-wrapper"> \r\n                <span class="ui-icon ui-icon-alert error-notice-icon"></span>[MSG]\r\n                </div>\r\n            </div>'.replace("[MSG]",
b("<div>").text(a.not_found_msg||"Not Found").html())).hide().prop("id",c+"_error")),b("#"+c+"_error").show("slow"))})});h.autocomplete({focus:function(a,b){return!1},source:p,minLength:a.minLength,delay:a.delay,select:function(b,c){h.data({chkid:c.item.chkid,display:c.item[a.match_prop]})}}).keypress(function(a){"13"==a.keyCode&&(a.preventDefault(),h.autocomplete("close"),t.trigger("click"))})}
...
(function(){var b=jQuery;window.init_municipality_autocomplete=function(a,c,e){a.each(function(){var a={},f=this,h=this.id.replace("Web",""),k=b("#"+h),g=b(this).data("cm_info",{value:this.value,chkid:k[0].value}).autocomplete({focus:function(a,b){return!1},source:create_caching_source_fn(b,c,a),minLength:1,close:function(){g.is(":focus")||l()}}).keypress(function(a){"13"==a.keyCode&&(a.preventDefault(),g.autocomplete("close"))}),l=function(){var c=g.data("cm_info"),h=f.value,l;if(h)if(c&&c.chkid&&
c.value&&c.value===h)k[0].value=c.chkid;else{l=string_ci_ai(h);if(a.content&&(c=b.grep(a.content,function(a){return string_ci_ai(a.value)===l}),1===c.length)){k[0].value=c[0].chkid;g.data("cm_info",c[0]);return}setTimeout(function(){g[0].focus()},1);alert(e)}else k[0].value=""};g.blur(function(){g.autocomplete("widget").is(":visible")||l()})})}})();
